...
class ClubSalesPlugin {
	...
	public function enqueue_scripts() {
		...
		// Register and enqueue child users styles
    $child_styles_url = plugin_dir_url(__FILE__) . 'assets/css/cs-child-styles.css';
    wp_enqueue_style('cs-child-styles', $child_styles_url, array('cs-styles'), filemtime(plugin_dir_path(__FILE__) . 'assets/css/cs-child-styles.css'));

		// Register and enqueue scripts
		wp_register_script('chart-js', 'https://cdn.jsdelivr.net/npm/chart.js', array('jquery'), '3.7.0', true);
		wp_register_script('cs-scripts', CS_PLUGIN_URL . 'assets/js/cs-scripts.js', array('jquery', 'chart-js'), $version, true);
...
public function modify_dashboard_tabs($tabs) {
    ...
    if (in_array('club_child_user', $user_roles)) {
        ...
        if (isset($tabs['assign-product'])) {  // Changed from 'products'
            $child_tabs['assigned-products'] = array(
                'icon' => 'dashicons-store',
                'title' => 'Your Products',
                'content_callback' => array('CS_Child_Shortcodes', 'assigned_products_tab')
            );
        }
        ...
        // Add sales material tab for child users
